...
class FragmentCheck : Fragment() {
    ...
    override fun onCreateView(
        ...
        Log.d("activityCHeck", "gender: $gender")
        Log.d("activityCHeck", "location: $location")
        Log.d("activityCHeck", "haveChild: $haveChild")
        Log.d("activityCHeck", "$activityArr")
    ...
    override fun onDestroy() {
        super.onDestroy()
        val pref: SharedPreferences = this.requireActivity().getSharedPreferences("$filename", AppCompatActivity.MODE_PRIVATE)
        val editor = pref.edit()

        val arr: ArrayList<ClassCheckTotal> = arrayListOf()
        val jsonArr = JSONArray()
        for (i in arr) {
            jsonArr.put(i)
        }
        val result = jsonArr.toString()
        Log.d("checklist", result)
        editor.putString("checklist", result).apply()
    }
    ...
    fun inputItem(
        ...
        Log.d("checklistBoolean", "filename: $filename")

        for (element in array) {
            val check = pref.getBoolean(element, false)
            checklist.add(ClassCheckStatus(element, check))
            Log.d("checklistBoolean", "element: $element, check, $check")
        }
